...
module S05_RankNTypes.RankNTypes where

 {- in number 5, we'll be discussing /higher rank
...
wrappy :: (forall x. x -> f x) -> (a, b) -> (f a, f b) 
wrappy f (x, y) = (f x, f y)

ex1 :: (Maybe Int, Maybe String) 
ex1 = wrappy Just (100, "hmm")

ex2 :: (Either () String, Either () Int) 
ex2 = wrappy Right ("ten", 10)

wrappyNum :: (Num a, Num b) => (forall x. Num x => x -> f x) -> (a, b) -> (f a, f b) 
wrappyNum f (a, b) = (f a, f b)

ex3 :: ([Int], [Double]) 
ex3 = wrappyNum (\x -> [x,1,2]) (500, 33.3)
