...
module RankNTypes.Exercises where
...
-- Nothing really, as we don't know anything about the type, no constraints...
...
lengthCanfold :: CanFold a -> Int 
lengthCanfold = unpackCanFold length 
...
filterEqs :: (forall a. Eq a => a -> Bool) -> [EqPair] -> [EqPair]
filterEqs p = foldr (\e es -> if fst (pred e) && snd (pred e) then e : es else es) [] 
  where 
    pred e = unpackEqPair p e 
...
-- pairs?
unpackTwoEP :: (forall a. Eq a => a -> r) -> EqPair -> EqPair -> [(r,r)]
unpackTwoEP f e1 e2 = [unpackEqPair f e1, unpackEqPair f e2]

comparePairs :: (forall a. Eq a => a -> Bool) -> EqPair -> EqPair -> Bool 
comparePairs p e1 e2 
  | a == c && b == d = True 
  | otherwise        = False
  where 
    [(a, b), (c, d)] = unpackTwoEP p e1 e2
...
data Component input output = Component input output
...
unpackNestedX :: NestedX i o -> (forall si so. si -> so -> r) -> r
unpackNestedX (NestedX (Component si so) f g) h = h si so
...
toNat :: SNat n -> Nat
toNat SZ = error "You should already know this one ;)"
