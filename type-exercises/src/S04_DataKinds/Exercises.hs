...
module S04_DataKinds.Exercises where
...
-- want.
newtype Sum a = Sum { a :: Int }
newtype Product b = Product { b :: Int }
...
-- Because Product and Sum are not type variables
...
-- 'Void'?
--
-- No, as there are no data constructors to promote that would be of kind Void, 
-- unlike Natural, where the data constructors are promoted and we get 'Zero :: Natural... 
...
--
-- To ensure certain types don't have a term level representation?
...
data StringAndIntList (stringCount :: Nat) (intCount :: Nat) where
  ...
  ConsInt    :: Int -> StringAndIntList s i -> StringAndIntList s ('S i)
  ConsString :: String -> StringAndIntList s i -> StringAndIntList ('S s) i

-- | b. Update it to keep track of the count of strings /and/ integers.

-- | c. What would be the type of the 'head' function?
headSI :: StringAndIntList s i -> Either String Int
headSI (ConsString s _) = Left s
headSI (ConsInt i _) = Right i
...
data MaybeConstrained (constraint :: Type -> Constraint) (isConstrained :: Bool) where 
  CanDo :: constraint a => a -> MaybeConstrained constraint 'True 
  NoCan :: a -> MaybeConstrained constraint 'False
...
-- on 'HList'.
tailHL :: HList ('Cons t ts) -> HList ts 
tailHL (HCons _ xs) = xs

-- | c. Could we write the 'take' function? What would its type be? What would
-- get in our way?
-- HNil and HCons will have different types, this won't work for the base case: 
-- takeHL :: Int -> HList ('Cons t ts) -> HList ('Cons t ts)
-- takeHL 0 _ = HNil ...
